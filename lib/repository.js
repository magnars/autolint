...
function relayDirtyEvent(fileName, errors) {
...
function listenTo(linter) {
  ...
  linter.on('dirty', relayDirtyEvent.bind(this));
  linter.on('dirty', checkForIntroducedErrors.bind(this));
...
function scan(paths) {
  ...
  self.findAllFiles(paths).then(function (files) {
    self.checkFiles(files).then(resolve(promise));
  });
  return promise;
}
...
function checkFiles(files) {
  var promise = buster.promise.create();
  all(files.map(checkLint.bind(this))).then(resolve(promise));
  return promise;
}
